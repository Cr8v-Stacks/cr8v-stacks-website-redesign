...
## 🛠️ Table of Contents & `wp_head()` Plugin Stylesheet Resolution (2026-08-12)

### 🔍 Root Cause Identified: Missing `wp_head()` in `header-blog.php`

#### 1. How WordPress Plugins Load Styles
In WordPress, plugins (such as Table of Contents plugins like `wptw`, `ez-toc`, etc.) enqueue their CSS stylesheets using WordPress's `wp_enqueue_style()` API. 

These stylesheets are automatically injected into the `<head>` of the page when WordPress fires the **`wp_head()`** hook inside your theme's header file.

#### 2. What Happened to `single.php` / `header-blog.php`
1. [`single.php`](file:///c:/Users/HP/Downloads/Mega%20Menu/wp-theme/cr8v-stacks/single.php) calls `get_header('blog')`, which loads [`header-blog.php`](file:///c:/Users/HP/Downloads/Mega%20Menu/wp-theme/cr8v-stacks/header-blog.php).
2. [`header-blog.php`](file:///c:/Users/HP/Downloads/Mega%20Menu/wp-theme/cr8v-stacks/header-blog.php) delegates to [`parts/header-blog.php`](file:///c:/Users/HP/Downloads/Mega%20Menu/wp-theme/cr8v-stacks/parts/header-blog.php).
3. In [`parts/header-blog.php`](file:///c:/Users/HP/Downloads/Mega%20Menu/wp-theme/cr8v-stacks/parts/header-blog.php#L63), the file starts directly with `<div class="c8bm-root">`. 
4. **It was missing the standard HTML document head wrapper**:
   ```php
   <!DOCTYPE html>
...
#### 3. Why DevTools Showed No CSS
Because `wp_head()` was never called in `header-blog.php`:
* WordPress **never outputted** the plugin's CSS `<link rel="stylesheet">` tags (such as `wptw-toc.css`) into the HTML document.
* The browser received the TOC HTML markup generated by the plugin in the post content, but **none of the plugin's CSS files existed in the document**.
* When inspecting `.wptw-toc-wrapper`, `.wptw-toc`, or any child elements in Chrome/Edge DevTools, the Styles panel showed **no CSS rules at all** because the stylesheet was literally never loaded into the DOM.

#### 4. Resolution Applied
Updated [`header-blog.php`](file:///c:/Users/HP/Downloads/Mega%20Menu/wp-theme/cr8v-stacks/header-blog.php) to include standard document head markup with `wp_head()`, `body_class()`, and `wp_body_open()` hooks:
```php
<?php
...
#### 5. Theme Audit Results
Audited all template headers and pages (`header.php`, `header-blog.php`, `front-page.php`, `page-service-*.php`, `single.php`, `single-case_study.php`):
